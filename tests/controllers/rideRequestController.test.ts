...
describe('rideRequestController', () => {
  ...
  describe('upsertRideRequestByKey - PUT idempotency', () => {
    it('should ensure only 1 row exists and message updated on duplicate calls', async () => {
      ...
      const upsertStub = sandbox.stub(rideRequestService, 'upsertByKey').resolves(mockRequest as any);
      ...
      await upsertRideRequestByKey(req, res, makeNext(res));
      req.body.message = 'Updated message';
      await upsertRideRequestByKey(req, res, makeNext(res));

      expect(upsertStub.callCount).to.equal(2);
      expect(res.json.calledWithMatch({ success: true, data: mockRequest })).to.be.true;
    });
  });

  describe('respondToRideRequest - Permission check', () => {
    it('should return 403 when wrong actorUserId tries to respond', async () => {
      sandbox.stub(rideRequestService, 'respond').rejects(
        new AppError(403, 'Only the recipient can respond to this request')
      );

      const req: any = {
        params: { id: 'req1' },
        body: { status: 'ACCEPTED' },
        user: { id: 'user3', role: 'BOTH' }
      };
      const res: any = { status: sinon.stub().returnsThis(), json: sinon.stub() };

      await respondToRideRequest(req, res, makeNext(res));
  ...
  describe('Contact info visibility', () => {
    it('should reveal contact info only when status is ACCEPTED', async () => {
      const prismaMock = {
        user: { findMany: sinon.stub(), findUnique: sinon.stub() },
        rideRequest: {
          upsert: sinon.stub(), findUnique: sinon.stub(),
          update: sinon.stub(), findMany: sinon.stub()
        }
      };
      ...
      const mockInboxRequests = [
        {
          ...
          fromUser: { id: 'user1', name: 'User 1', contactType: 'EMAIL', contactValue: 'user1@example.com' }
        },
        {
          id: 'req2', status: RideRequestStatus.REJECTED,
          fromUser: { id: 'user2', name: 'User 2', contactType: 'PHONE', contactValue: '1234567890' }
        }
      ];
      const mockOutboxRequests = [
        {
          id: 'req3', status: RideRequestStatus.ACCEPTED,
          toUser: { id: 'user3', name: 'User 3', contactType: 'WECHAT', contactValue: 'user3_wechat' }
        },
        {
          id: 'req4', status: RideRequestStatus.PENDING,
          toUser: { id: 'user4', name: 'User 4', contactType: 'EMAIL', contactValue: 'user4@example.com' }
        }
      ];

      prismaMock.rideRequest.findMany.resolves(mockInboxRequests);
      const inboxReq: any = { params: { userId: 'user5' } };
      const inboxRes: any = { status: sinon.stub().returnsThis(), json: sinon.stub() };
      ...
      const outboxRes: any = { status: sinon.stub().returnsThis(), json: sinon.stub() };
