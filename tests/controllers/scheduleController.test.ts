...
describe('scheduleController', () => {
  ...
  beforeEach(() => {
    ...
    const moduleKeys = require.cache ? Object.keys(require.cache).filter(key => 
      key.includes('scheduleController') || key.includes('lib/prisma')
    ) : [];
  ...
  describe('deleteScheduleEntry', () => {
    it('should return 404 if entry not found', async () => {
      const req = mockReq({ params: { entryId: 'nonexistent-entry' } });
      const res = mockRes();
    ...
    it('should delete schedule entry successfully', async () => {
      const req = mockReq({ params: { entryId: 'entry-id' } });
      const res = mockRes();
  ...
  describe('createWeeklySchedule', () => {
    ...
    it('should create weekly schedule successfully', async () => {
      ...
      transactionMock.callsFake(async (callback: any) => {
        const tx = { scheduleEntry: { upsert: sinon.stub() } };
      ...
      expect(transactionMock.called).to.be.true;
    });
  });
});